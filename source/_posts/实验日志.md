...
补充：在networks_v2.py的基础上，在res、ince_res、den三个网络中的块结构中的卷积块结构中添加调整通道数的（1, 1, 1）大小的卷积（其中den网络没有添加），并使用PReLU来代替ReLU，在res模型中，还将卷积下采样改为了中心池化，保存为networks_v3.py

补充：networrks_v3.py中的模型在LUNA16数据上训练结果
其中vgg模型单折FROC评分为82.6
其中res模型单折FROC评分为未知（尚未完成）
其中den模型单折FROC评分为85.3
其中ince_res模型单折FROC评分为未知（尚未完成）
...
### 2018-09-03 尝试在LUNA16数据上进行hard-mining:

使用networks_v2.py中的ince_res网络，首先随机采样40000正样本，随机选出150000负样本，然后正样本旋转扩充为四倍作为初始训练数据，每次迭代后，学习速率回到最初学习速率继续训练
初始训练结果FROC：76.7
第一次迭代后训练结果FROC：74.3

使用networks_v3.py中的ince_res网络，首先随机采样40000正样本，随机选出150000负样本，然后正样本旋转扩充为四倍作为初始训练数据，每次迭代后，学习速率接着上次训练最终的速率继续衰减
初始训练结果FROC：78.3
第一次迭代后训练结果FROC：81.2

新增代码目录：

     老服务器:~/Code/MULTI_SIZE_MULTI_MODEL_HARD_MINING_FPR/

数据：

     老服务器:/mnt/data4/lty/MULTI_SIZE_MULTI_MODEL_HARD_MINING_FPR/data/

结果目录：

     老服务器:/mnt/data4/lty/MULTI_SIZE_MULTI_MODEL_HARD_MINING_FPR/